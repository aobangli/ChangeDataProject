...
def main():
    late_columns = ["duration", "number_of_message", "number_of_revision",
                    "avg_delay_between_revision", "weighted_approval_score"]

    features_list = initial_feature_list + late_columns
    file_header = ["project", "change_id", 'created', 'subject'] + features_list + ['status']
    output_file_name = f"{root}/{project}_multiple_revisions.csv"
    ...
    for change_number in tqdm(change_numbers):
        ...
        created = pd.to_datetime(change.created)
        calculator = FeatureCalculator(change, created)
        description_features = calculator.description_features
        weighted_approval_score = avg_delay_between_revisions = 0
        status = calculator.status

        for revision_no, revision in enumerate(change.revisions):
            current_date = pd.to_datetime(revision.created)
            calculator.set_current_date(current_date)

            author_features = calculator.author_features
            reviewer_features = calculator.reviewer_features

            num_of_messages = len([message for message in change.messages if message.date <= revision.created])

            file_features = calculator.file_features
            project_features = calculator.project_features
            duration = day_diff(current_date, created)

            if revision_no:
                avg_delay_between_revisions = day_diff(current_date, change.first_revision.created) / (revision_no + 1)

                for label in change.labels:
                    if revision.created >= label.date >= change.revisions[revision_no - 1].created:
                        weighted_approval_score += float(revision_no) * label.value / (revision_no + 1)

            feature_vector = [
                change.project, change.change_number, current_date, change.subject,
                ...
                # multiple revision features
                duration, num_of_messages, revision_no + 1, avg_delay_between_revisions,
    ...
    df = df.sort_values(by=['created', 'change_id'])
    df.to_csv(output_file_name, index=False, float_format='%.2f')

    initial_columns = ["project", "change_id", 'created', 'subject'] + initial_feature_list + ['status']

    df = df[df["number_of_revision"] == 1][initial_columns]
    df.to_csv(f"{root}/{project}.csv", index=False, float_format='%.2f')
...
class FeatureCalculator:
    ...
    def set_current_date(self, date):
        ...
        self.old_date = date - timedelta(days=lookback)

    @property
    def status(self):
        return 1 if self.change.status.upper() == 'MERGED' else 0
